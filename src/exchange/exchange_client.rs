...
use crate::{
    exchange::{
        actions::{
            ApproveBuilderFee, BulkCancel, BulkModify, BulkOrder, ClaimRewards, EvmUserModify,
            ScheduleCancel, SendAsset, SetReferrer, UpdateIsolatedMargin, UpdateLeverage, UsdSend,
        },
    ...
    BaseUrl, BulkCancelCloid, ClassTransfer, DynSigner, Error, ExchangeResponseStatus, SpotSend,
    SpotUser, VaultTransfer, Withdraw3,
};

pub struct ExchangeClient {
    pub http_client: HttpClient,
    pub wallet: Arc<DynSigner>,
    pub meta: Meta,
...
impl ExchangeClient {
    ...
    pub async fn enable_big_blocks(
        ...
        wallet: Option<Arc<DynSigner>>,
    ) -> Result<ExchangeResponseStatus> {
        let wallet = wallet.unwrap_or_else(|| self.wallet.clone());
        ...
        let signature = sign_l1_action(wallet.as_ref(), connection_id, is_mainnet).await?;
    ...
    pub async fn usdc_transfer(
        ...
        let signature = sign_typed_data(&usd_send, wallet.as_ref()).await?;
    ...
    pub async fn send_asset(
        ...
        let signature = sign_typed_data(&send_asset, wallet.as_ref()).await?;
    ...
    pub async fn market_close(
        ...
        let slippage = params.slippage.unwrap_or(0.05); // Default 5% slippage
        let wallet = params.wallet.unwrap_or_else(|| self.wallet.clone());
    ...
    pub async fn withdraw_from_bridge(
        ...
        let signature = sign_typed_data(&withdraw, wallet.as_ref()).await?;
    ...
    pub async fn spot_transfer(
        ...
        let signature = sign_typed_data(&spot_send, wallet.as_ref()).await?;
    ...
    pub async fn approve_builder_fee(
        ...
        let signature = sign_typed_data(&approve_builder_fee, wallet.as_ref()).await?;
...
mod tests {
    ...
    async fn test_limit_order_action_hashing() -> Result<()> {
        ...
        let signature = sign_l1_action(&wallet, connection_id, true).await?;
        ...
        let signature = sign_l1_action(&wallet, connection_id, false).await?;
    ...
    #[tokio::test]
    async fn test_approve_builder_fee_signing() -> Result<()> {
        let wallet = get_wallet()?;
        ...
        let mainnet_signature = sign_typed_data(&mainnet_fee, &wallet).await?;
        assert_eq!(
        ...
        let testnet_signature = sign_typed_data(&testnet_fee, &wallet).await?;
        assert_eq!(
    ...
    #[tokio::test]
    async fn test_approve_builder_fee_hash() -> Result<()> {
    ...
    #[tokio::test]
    async fn test_claim_rewards_action_hashing() -> Result<()> {
        let wallet = get_wallet()?;
        let action = Actions::ClaimRewards(ClaimRewards {});
        let connection_id = action.hash(1583838, None)?;
    ...
    #[tokio::test]
    async fn test_send_asset_signing() -> Result<()> {
        let wallet = get_wallet()?;
        ...
        let mainnet_signature = sign_typed_data(&mainnet_send, &wallet).await?;
        ...
        let testnet_signature = sign_typed_data(&testnet_send, &wallet).await?;
        // Verify signatures are different for mainnet vs testnet
        assert_ne!(mainnet_signature, testnet_signature);
        ...
        let vault_signature = sign_typed_data(&vault_send, &wallet).await?;
